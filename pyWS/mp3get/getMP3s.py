...
root = Path('./pyWS/mp3get/mp3s')

for n, fuda in enumerate(hiraFuda):
    time.sleep(6)
    rq = f'https://api.tts.quest/v1/voicevox/?text={fuda}&speaker=1'
    respnse = requests.get(rq)
    data = respnse.json()
    mp3_url = data['mp3DownloadUrl']

    time.sleep(6)
    name_num = n + 1
    file_name = f'{name_num:03}.mp3'
    res_mp3 = requests.get(mp3_url)
    Path(root, file_name).write_bytes(res_mp3.content)
    time.sleep(3)
    _size = os.path.getsize(Path(root, file_name))
    print(f'size: {_size}    {file_name}')
